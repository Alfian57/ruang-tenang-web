...
import { ArrowLeft, Send, Trash2, Clock, Heart, MessageSquare, CheckCircle2, Trophy, AlertTriangle, ShieldAlert, MoreVertical, Ban } from "lucide-react";
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {user && user.id !== forum.user_id && (
                  <DropdownMenuItem
                    className="text-red-600 focus:text-red-600 focus:bg-red-50 cursor-pointer"
                    onClick={() => handleToggleForumAccess(forum.user_id)}
                  >
                    <Ban className="w-4 h-4 mr-2" />
                    Blokir Akses Forum (Admin)
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto p-4 lg:p-6 space-y-6">
            ...
            <div className="space-y-4 pb-4">
              {posts.map((post) => {
                const isAcceptedAnswer = post.is_accepted_answer ?? post.is_best_answer ?? false;
                const postLikesCount = post.upvotes_count ?? post.likes_count ?? 0;

                return (
                  <div key={post.id} className={cn("flex gap-3 group transition-all", isAcceptedAnswer && "translate-x-2")}>
                    ...
                    <div className={cn(
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            {isAcceptedAnswer ? "Unmark" : "Best Answer"}
                          </Button>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {user && user.id !== post.user_id && (
                                <DropdownMenuItem
                                  className="text-red-600 focus:text-red-600 focus:bg-red-50 cursor-pointer"
                                  onClick={() => handleToggleForumAccess(post.user_id)}
                                >
                                  <Ban className="w-4 h-4 mr-2" />
                                  Blokir Akses Forum (Admin)
                                </DropdownMenuItem>
                      ...
                      <div className="flex items-center gap-3 border-t border-gray-100/50 pt-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded p-1 text-xs font-medium text-gray-500 transition-colors hover:bg-gray-50"
                        >
