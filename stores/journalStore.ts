...
export const useJournalStore = create<JournalState & JournalActions>((set, get) => ({
  ...
  loadJournals: async (token: string, page = 1, limit = 10) => {
    ...
    try {
      ...
      const response = await api.listJournals(token, {
        page,
        limit,
  ...
  loadJournal: async (token: string, id: number) => {
    set({ isLoading: true, error: null });
    try {
      const response = await api.getJournal(token, id);
  ...
  createJournal: async (token, data) => {
    ...
    try {
      const response = await api.createJournal(token, data);
  ...
  updateJournal: async (token, id, data) => {
    set({ isSaving: true, error: null });
    try {
      const response = await api.updateJournal(token, id, data);
  ...
  searchJournals: async (token, query) => {
    ...
    try {
      ...
      const response = await api.searchJournals(token, query);
  ...
  loadSettings: async (token) => {
    try {
      const response = await api.getJournalSettings(token);
  ...
  updateSettings: async (token, data) => {
    set({ isSaving: true, error: null });
    try {
      const response = await api.updateJournalSettings(token, data);
  ...
  toggleAIShare: async (token, journalId) => {
    try {
      const response = await api.toggleJournalAIShare(token, journalId);
      const updatedJournal = response.data;
      set((state) => ({
  ...
  loadAIContext: async (token) => {
    try {
      const response = await api.getJournalAIContext(token);
  ...
  loadAIAccessLogs: async (token) => {
    try {
      const response = await api.getJournalAIAccessLogs(token);
  ...
  loadWritingPrompt: async (token, mood) => {
    try {
      const response = await api.getJournalWritingPrompt(token, mood);
  ...
  loadWeeklySummary: async (token) => {
    try {
      const response = await api.getJournalWeeklySummary(token);
  ...
  loadAnalytics: async (token) => {
    try {
      const response = await api.getJournalAnalytics(token);
  ...
  exportJournals: async (token, format, startDate, endDate) => {
    ...
    try {
      const response = await api.exportJournals(token, format, startDate, endDate);
