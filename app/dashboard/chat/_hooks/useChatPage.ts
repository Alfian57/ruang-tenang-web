...
export function useChatPage() {
  ...
  const handleStartNewChat = () => {
    // GPT/Gemini/Claude-style: starting a new chat just clears the active
    // session and shows the empty state with the input. The actual session is
    // created automatically when the user sends their first message.
    clearActiveSession();
  ...
  const handleSendText = async (content: string) => {
    ...
    // one on the fly (title will be auto-generated by the backend from this
  ...
  const handleSendAudio = async (audioBlob: Blob) => {
    ...
    await sendAudioMessage(token, audioBlob, buildMessageOptions("voice_input"));
  };
  ...
  const handleSuggestedPrompt = async (prompt: string) => {
    ...
    // handleSendText auto-creates a session when none is active and the
    // backend auto-generates the title from this first message.
