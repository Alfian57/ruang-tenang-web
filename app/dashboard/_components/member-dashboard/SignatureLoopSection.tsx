...
export function SignatureLoopSection({ viewModel }: SignatureLoopSectionProps) {
  ...
  return (
    <motion.section
      ...
      className="rounded-3xl border border-primary/20 bg-linear-to-br from-primary/10 via-white to-primary/10 p-5 md:p-6 shadow-sm"
    >
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-widest text-primary/80">Siklus Unggulan</p>
          <h2 className="font-brand-display text-xl md:text-2xl font-bold text-gray-900 mt-1">
            Refleksi {"->"} Progres {"->"} Hadiah {"->"} Komunitas
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Penyelesaian Siklus</p>
      ...
      <div className="mt-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        {loopStages.map((stage) => (
          <Link key={stage.key} href={stage.href}>
            <div className="rounded-2xl border border-white bg-white/85 p-3.5 hover:shadow-md hover:border-primary/20 transition-all h-full">
              <div className="flex items-center justify-between gap-2">
                ...
                >
                  {stage.completed ? "Selesai" : "Selanjutnya"}
                </span>
              </div>
