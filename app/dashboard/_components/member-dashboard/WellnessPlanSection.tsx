...
export function WellnessPlanSection({ viewModel }: WellnessPlanSectionProps) {
  ...
  return (
    <section data-user-tour="user-wellness-plan" className="rounded-2xl border border-primary/20 bg-white p-5 shadow-sm">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(22rem,0.9fr)]">
        <div className="min-w-0">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-primary">Rencana Kesejahteraan Personal</p>
            ...
            <div className="rounded-2xl border border-primary/20 bg-primary/10 px-4 py-3 text-right">
              <p className="text-xs font-semibold uppercase tracking-wide text-primary">Progres</p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {(wellnessPlan?.items ?? []).slice(0, 4).map((item) => {
              ...
              return (
                <div key={item.id} className={`rounded-2xl border p-3 ${isDone ? "border-primary/20 bg-primary/10" : "border-gray-200 bg-gray-50"}`}>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Hari {item.day_number}</span>
                    {isDone ? <CheckCircle2 className="h-4 w-4 text-primary/80" /> : <span className="h-2 w-2 rounded-full bg-amber-400" />}
                  </div>
            ...
            {(!wellnessPlan || wellnessPlan.items.length === 0) && (
              <div className="rounded-2xl border border-dashed border-primary/20 bg-primary/10 p-4 text-sm text-primary sm:col-span-2 xl:col-span-4">
                Pengenalan kesejahteraan belum selesai. Setelah selesai, rencana 7 hari akan muncul otomatis di sini.
              </div>
            )}
          </div>
        </div>
        ...
        <aside className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
          ...
          <p className="mt-1 text-sm leading-6 text-gray-600">
            {upcomingWellnessItem?.description ?? "Jawab beberapa pertanyaan singkat agar dasbor bisa menyusun alur yang lebih terarah."}
          </p>
